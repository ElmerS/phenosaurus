...
def listgenes(request):
	...
	assert isinstance(request, HttpRequest)
	authorized_screens = list(
		get_authorized_screens(request))  # Check user groups to see which screen are allowed to be seen
	context = {
		'data': cf.list_genes(authorized_screens), # Call custom fucnction to draw the table
		'year': datetime.now().year,
	}
...
def opengenefinder(request):
	...
	hidelegend = request.GET.get('hidelegend', '')	# Whether a table should be drawn with raw values

	context = {'filter':filter, 'year': datetime.now().year, 'title': 'Study the effect of genes across multiple phenotypes'}
	if request.GET:
		if not screenids:
		...
		if set(cf.set_screenids(screenids)).issubset(set(authorized_screens)):
			...
			hidelegend = True if hidelegend=='on' else False
			...
			# be drawn. Raise an error. The error is already generated by cf.create_genes_array.
				...
				context['title'] = ' '.join(['Results of', ', '.join(genes_array), 'on', str(len(screenids_array)), 'phenotypes in HAP1 cells'])
				df, error, context['text'] = cf.df_multiple_geneplot(genes_array, screenids_array, pvcutoff, authorized_screens)
				if not df.empty:
					plotlist = plots.single_gene_plots(df, hidelegend) # Call single_gene_plots to create a list of plotting objects
